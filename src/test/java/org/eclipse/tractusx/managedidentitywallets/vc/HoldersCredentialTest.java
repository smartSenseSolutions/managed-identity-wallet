...
class HoldersCredentialTest {
    ...
    @Autowired
    private IssuersCredentialController issuersCredentialController;
    ...
    void validateCredentialsWithInvalidVC() throws com.fasterxml.jackson.core.JsonProcessingException {
        //data setup
        VerifiableCredential credential = issueVC();
    ...
    void validateCredentialsWithRevocationTure() throws JsonProcessingException {
        ...
        try (MockedStatic<LinkedDataProofValidation> utils = Mockito.mockStatic(LinkedDataProofValidation.class)) {
            ...
            Map<String, Object> stringObjectMap = credentialController.credentialsValidation(verifiableCredential, false, true).getBody();
            Assertions.assertTrue(Boolean.parseBoolean(stringObjectMap.get(StringPool.VALID).toString()));
            ...
            Mockito.when(revocationClient.verify(Mockito.any(StatusVerificationRequest.class))).thenReturn(statusVerificationResponse);
            stringObjectMap = credentialController.credentialsValidation(verifiableCredential, false, true).getBody();
            Assertions.assertFalse(Boolean.parseBoolean(stringObjectMap.get(StringPool.VALID).toString()));
    ...
    @Test
    @DisplayName("Revoke VC with invalid token/issuer. Issuer of VC is not caller of API. It should give 403")
    void revokeVCWithInvalidBpnAccess() throws JsonProcessingException {
        String bpn = UUID.randomUUID().toString();

        String baseBpn = miwSettings.authorityWalletBpn();
        Wallet wallet = TestUtils.getWalletFromString(TestUtils.createWallet(bpn, "Test", restTemplate, baseBpn).getBody());


        VerifiableCredential verifiableCredential1 = TestUtils.issueRandomVC(wallet.getDid(), miwSettings.authorityWalletDid(), miwSettings, objectMapper, revocationClient, restTemplate);
        Map<String, Object> map = objectMapper.readValue(verifiableCredential1.toJson(), Map.class);

        HttpHeaders headersWithInvalidBpn = AuthenticationUtils.getValidUserHttpHeaders(UUID.randomUUID().toString());

        HttpEntity<Map> entity = new HttpEntity<>(map, headersWithInvalidBpn);
        ResponseEntity<Void> voidResponseEntity = restTemplate.exchange(RestURI.CREDENTIALS_REVOKE, HttpMethod.POST, entity, Void.TYPE);
        Assertions.assertEquals(HttpStatus.FORBIDDEN.value(), voidResponseEntity.getStatusCode().value());
    }

    @Test
    @DisplayName("Revoke VC should return 200")
    void revokeVC200() throws JsonProcessingException {
        //issue revocable VC
        String bpn = UUID.randomUUID().toString();
        String did = DidWebFactory.fromHostnameAndPath(miwSettings.host(), bpn).toString();
        String type = "TestCredential";
    ...
    @Test
    @DisplayName("Revoke VC should return 400")
    void revokeVC400() throws JsonProcessingException {
        //issue revocable VC
        String bpn = UUID.randomUUID().toString();
        String did = DidWebFactory.fromHostnameAndPath(miwSettings.host(), bpn).toString();
        String type = "TestCredential";
        ...
        try (MockedStatic<LinkedDataProofValidation> utils = Mockito.mockStatic(LinkedDataProofValidation.class)) {
            ...
            Assertions.assertThrows(CredentialValidationProblem.class, () -> {
                try {
                    ...
                    Assertions.assertFalse(Boolean.parseBoolean(validationResults.get(0).get(StringPool.REVOKED).toString()));
